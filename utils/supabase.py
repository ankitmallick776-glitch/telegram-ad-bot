...
class SupabaseDB:
    ...
    async def init_table(self):
        """Initialize and test table connection"""
        try:
    ...
    async def get_user(self, user_id: int):
        ...
        try:
            ...
            logger.error(f"Get user {user_id} error: {e}")
            return None
    ...
    async def get_referrer_by_code(self, referral_code: str):
        ...
        try:
            ...
            logger.error(f"Get referrer {referral_code} error: {e}")
            return None
    ...
    async def user_already_referred(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Check referral {user_id} error: {e}")
            return False
    ...
    async def create_user_if_not_exists(self, user_id: int, username: str = ""):
        ...
        try:
            ...
            logger.info(f"👤 CREATED new user {user_id} with code {referral_code}")
        except Exception as e:
            logger.error(f"Create user {user_id} error: {e}")
    ...
    async def add_balance(self, user_id: int, amount: float) -> bool:
        """Add amount to user balance (returns success)"""
        current = await self.get_balance(user_id)
        new_balance = current + amount
        try:
            self.client.table("users").update({"balance": new_balance}).eq("user_id", user_id).execute()
            logger.info(f"💰 User {user_id}: +{amount} = {new_balance}")
            return True
        except Exception as e:
            logger.error(f"Balance update {user_id} error: {e}")
            return False
    ...
    async def give_daily_bonus(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.info(f"🎁 Daily bonus given to {user_id}")
            return True
        except Exception as e:
            logger.error(f"Daily bonus {user_id} error: {e}")
            return False
    ...
    async def process_referral(self, user_id: int, referrer_code: str):
        ...
        if await self.user_already_referred(user_id):
            logger.warning(f"❌ EXPLOIT BLOCKED: User {user_id} already referred")
            return False
        ...
        try:
            ...
            if referrer and referrer["user_id"] != user_id:
                ...
                try:
                    ...
                    logger.info(f"📊 Referral history: {user_id} → {referrer_id}")
                except Exception as e:
                    logger.warning(f"History insert error: {e}")
                logger.info(f"✅ REFERRAL: {user_id} → {referrer_id} (+40 Rs)")
                return True
        except Exception as e:
    ...
    async def add_referral_commission(self, new_user_id: int, reward: float) -> None:
        ...
        try:
            ...
            if response.data:
                ...
                logger.info(f"🤝 COMMISSION: {new_user_id} ({reward}) → {referrer_id} ({commission:.2f})")
        except Exception as e:
    ...
    async def create_or_update_daily_task(self, user_id: int, tasks_completed: int = 0, pending_reward: float = 0):
        ...
        try:
            ...
            logger.info(f"📋 Tasks {user_id}: {tasks_completed}/4, {pending_reward} Rs")
        except Exception as e:
            logger.error(f"Task update {user_id} error: {e}")
    ...
    async def mark_code_used(self, code_id: int, user_id: int):
        ...
        try:
            ...
            logger.info(f"✅ Code {code_id} used by {user_id}")
        except Exception as e:
            logger.error(f"Mark code error: {e}")
    ...
    async def generate_daily_codes(self):
        ...
        try:
            ...
            if response.data and len(response.data) >= 3:
                logger.info("✅ Codes already generated for today")
                return
            # Delete old codes
            ...
            logger.info(f"📋 Generated codes: {[c['secret_code'] for c in codes]}")
            return codes
        except Exception as e:
